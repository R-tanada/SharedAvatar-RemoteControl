...
from mod.MotionControll import *
...
class CoreManager:
    def __init__(self):
        self.avatarPeerID = RC.AVATAR_PEER_ID
    ...
    def __peer_joined(self, name, path, data):
        ...
        if data not in self.joined_dict.values():
            ...
            motion_controll = MotionControll('liner')
            self.joined_dict[str(ID)][1] = motion_controll
        ...
        for i in self.joined_dict.keys():
            if resource == 'user-%s-data'%i:
                if channel == 'command':
                    ...
                    data['user'] = i
                    self.core_write_data('feedback-to-user', 'cursor', data)

                elif channel == 'cursor-pressed':
                    data['user'] = i
                    self.core_write_data('feedback-to-user', 'cursor-pressed', data)

                elif channel == 'button-state':
                    data['user'] = i
                    self.core_write_data('feedback-to-user', 'button-state', data)
